...
2. In each Activity, locate and analyze all lists, FAB components, and any fixed or sticky UI elements (e.g., headers, filter bars, or search bars) to detect which have existing edge-to-edge support. For every component without edge-to-edge support, plan to make each of these components edge-to-edge.
...
- The app **MUST** apply system insets, or align content to rulers, so critical
  ...
  1. **PREFERRED:** When available, use `Scaffold`s and pass `PaddingValues` to the content lambda. If the Scaffold wraps a navigation container (e.g., `NavHost`), pass the PaddingValues into your screen composables.
...
## Parent Composables with Fixed Composables and Scrolling Content

When a parent contains both fixed composables (e.g. FilterChip) and scrolling content (e.g. LazyColumn), apply insets as padding to the parent container so the UI block starts below the system bars:
...
Scaffold { innerPadding ->
    ...
    Column(
        ...
        // LazyColumn accounts for the bottom inset so the list scrolls behind the navigation bar
        // Apply start and end padding for landscape orientations where the display cutout and navigation bar might change position
        LazyColumn(
...
If the fixed content is collapsible, and you want the list to scroll behind the system bar when it collapses, use a `Box` to layer the fixed content over the list and dynamically measure the fixed element's height:
...
Scaffold
 { innerPadding ->
    ...
    // 1. Track the height of the fixed element dynamically
    var fixedHeightPx by remember { mutableIntStateOf(0) }
    val fixedHeightDp = remember(fixedHeightPx) {
        with(density) { fixedHeightPx.toDp() }
    }

    Box(modifier = Modifier.fillMaxSize()) {
        // 2. The List is the bottom layer, filling the screen.
        LazyColumn(
            modifier = Modifier.fillMaxSize(),
            contentPadding = PaddingValues(
                top = innerPadding.calculateTopPadding() + fixedHeightDp,
                bottom = innerPadding.calculateBottomPadding(),
                start = innerPadding.calculateStartPadding(layoutDirection),
                end = innerPadding.calculateEndPadding(layoutDirection)
            )
        ...
        // 3. The Fixed Element is placed on top, and collapses when the users scrolls
        // Measure its height to offset it
        Box(
            modifier = Modifier
                .offset {
                    IntOffset(x = 0, y = innerPadding.calculateTopPadding().roundToPx())
                }
                .onGloballyPositioned { coords ->
                    fixedHeightPx = coords.size.height
                }
        ) {
            FilterChip( ... )
        }
